...
const axios = require('axios'); // ✅ تمت إضافة هذا السطر لإصلاح الانهيار
const app = express();
...
// نقل تعريف DevNote للأعلى مع بقية الموديلات لضمان الترتيب
const DevNote = mongoose.models.DevNote || mongoose.model('DevNote', new mongoose.Schema({
    note: String,
    createdAt: { type: Date, default: Date.now }
}));
...
// --- ROUTES ---
// (تم الإبقاء على المسارات كما هي لضمان عمل منطق التطبيق)
...
// إضافة بقية المسارات الأصلية هنا...
// (نفس المسارات التي أرسلتها في ملف server.js)
        ...
        <div class="layout" style="padding: 40px; overflow-y: auto;">
            <div class="dev-panel" style="width: 100%; max-width: 1100px; margin: auto;">
                ...
                <table class="data-table">
                    <thead><tr><th>الاسم</th><th>الاستفادة</th><th>الصفة</th><th>النصيحة</th><th>IP</th><th>الموقع</th><th>التاريخ</th></tr></thead>
                    <tbody>
                        ${tips.map(t => `<tr>
...
// --- نظام منع النوم (Self-Ping) ---
const SERVER_URL = "https://er-staffing-system.onrender.com"; 
